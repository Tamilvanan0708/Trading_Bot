...
class AnalysisScheduler:
    ...
    async def _run_analysis(self, snap, latest_candle) -> None:
        ...
        async with async_session_factory() as session:
            ...
            try:
                # Custom user strategies (Fib Retracement, SMC With Fib, Fib Trend) are continuously
                # synchronized via sync_strategy_paper_trades() ensuring unified signal IDs and state handling.
                from app.paper_trading.sync import sync_strategy_paper_trades
                await sync_strategy_paper_trades(session)
