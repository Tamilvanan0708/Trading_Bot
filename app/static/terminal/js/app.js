...
function buildRichStrategyView(mount, endpoint, strategyName, strategySub, strategyType) {
  ...
  function renderTfButtons(d, curSelTf) {
    return TFS.map(tf => {
      ...
      if (hasPaperTrade) {
        ...
        activeBadge = `<span style="color:var(--text-dim);font-size:9px;font-weight:600;margin-left:4px">SCAN</span>`;
      }
  ...
  function renderLockMsg(d) {
    ...
    if (activeLockTf || (d.active_trade_tfs && d.active_trade_tfs.length > 0)) {
      const activeList = (d.active_trade_tfs && d.active_trade_tfs.length > 0) ? d.active_trade_tfs : [activeLockTf];
      const names = activeList.map(tf => TF_LABELS[tf] || tf.toUpperCase()).join(" · ");
      return `<span class="badge badge-green" style="font-size:12px;display:inline-flex;align-items:center;gap:5px"><span class="dot dot-green" style="animation:pulse 1.5s infinite"></span>🟢 ACTIVE: ${names}</span>`;
    }
    return `<span class="badge badge-blue" style="font-size:12px">⚡ MULTI-SLOT: 5M · 15M · 30M · 1H CONCURRENT SCANNING</span>`;
  }
